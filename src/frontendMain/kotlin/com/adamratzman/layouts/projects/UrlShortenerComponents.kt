        ...
        div(classes = nameSetOf(MarginAuto, MarginSmallBottom, WidthOneHalf)) {
            ...
            div {
                ...
                formPanel<ShortenedUrlDto>(classes = nameSetOf(MarginMediumBottom.asString)) {
                    ...
                    ) { pathText -> pathText.getValue() == null || pathText.getValue()?.let { it.length in 0..shortenedUrlPathMaxLength && it.all { char -> char.isAlphanumeric() } } == true }
                    ...
                    hPanel(spacing = 10) {
                        button("Generate") {
                            onClick {
                                ...
                                GlobalScope.launch {
                                    try {
                                        val generatedShortenedUrl = UrlShortener.insertShortenedUrl(this@formPanel.getData())
                            ...
                            onClick {
                                ...
                                GlobalScope.launch {
                                    try {
                                        ...
                                        val shortenedUrl = UrlShortener.getShortenedUrl(path)
                                        SiteManager.redirectToUrl(UrlShortenerViewSingleShortenedLink(shortenedUrl.path).devOrProdUrl())
                                    } catch (exception: ServiceException) {
                                        this@formPanel.getControl(FindUrlShortenerInfoForm::url)!!.validatorError = "Please enter a valid shortened URL."
                                    }
                                }
                            }
                        }
...
class UrlShortenerViewSingleShortenedLinkComponent(parent: Container) : SiteStatefulComponent(parent = parent, buildStatefulComponent = { state ->
    ...
    div(classes = nameSetOf(MarginMediumTop, PaddingRemoveBottom)) {
        GlobalScope.launch {
            try {
                val shortenedUrl = UrlShortener.getShortenedUrl(state.view.path)
