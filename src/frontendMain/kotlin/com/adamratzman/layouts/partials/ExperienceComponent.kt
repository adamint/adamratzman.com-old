...
class ExperienceComponent(val experienceWrapper: ResumeExperienceWrapper, parent: Container) : SiteStatefulComponent(parent = parent, buildStatefulComponent = {
    div(className = MarginSmallBottom.asString) {
        h4(classes = if (experienceWrapper.start != null) nameSetOf(MarginRemoveVertical, UkInline) else nameSetOf(MarginRemoveVertical)) {
            bold {
                if (experienceWrapper.locationUrl != null) link(label = experienceWrapper.name, url = experienceWrapper.locationUrl, className = "bold")
        ...
        dataContainer(experienceWrapper.experiences.toMutableList(), { experience, _, _ ->
            div {
                h4(
                        classes = if (experienceWrapper.start == null && !experienceWrapper.incoming) nameSetOf(MarginRemoveVertical, UkInline, MarginSmallLeft)
                        else nameSetOf(MarginRemoveVertical)
                ) {
                    style { fontStyle = ITALIC }

                    if (experience.nameTooltip != null) {
                        span(content = experience.name, className = "dashed") {
                ...
                if (experience.bullets?.isNotEmpty() == true) {
                    ul(classes = nameSetOf(MarginRemoveVertical, MarginMediumLeft, "width-80")) {
                        dataContainer(experience.bullets.toMutableList(), { bullet, _, _ ->
                            li(content = bullet, className = "rem-1-2")
                        })
                    }
                }
